...
** Last update Wed Jan  4 15:33:28 2017 Maxime JENNY
*/
				      ...
				      t_my_framebuffer *display, t_wolf *wolf)
{
  ...
  my_draw_line(display, from, to, my_create_color(211, 81, 0, 255));
}
			      ...
			      sfVector2f player, sfVector2i mapSize)
{
  ...
  i = -1;
  while (angle < (wolf->angle + (FOV / 2)) && ++i < WIDTH)
    {
	...
	return ;
      ray->angle = angle;
      if (wolf->tog[1] == 1)
	ray->dist = ray->dist * cos((angle - wolf->angle) * M_PI / 180);
  ...
  while (i < HEIGHT / 2)
    {
      ...
      to.x = WIDTH;
      to.y = i;
      my_draw_line(display, from, to, my_create_color(0, 167, 255, c));
      i++;
      (c > 0 & (i % 2) == 0) ? (c--) : (0);
    }
}
...
void		my_draw_wolf(t_wolf *wolf, t_my_framebuffer *display)
{
  ...
  my_draw_square(display, my_create_vector((WIDTH / 2) - 1, (HEIGHT / 2) - 1),
		 my_create_vector(2, 2), sfBlack);
  if (wolf->tog[3] == 1)
    {
