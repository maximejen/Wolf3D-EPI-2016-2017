...
** Last update Fri Jan  6 00:33:54 2017 Maxime JENNY
*/
			...
			sfVector2i *mapSize)
{
  ...
  my_find_coords2(a, b, direct, pos);
  d1 = sqrt((a->u_x * a->u_x) + (a->u_y * a->u_y));
  d2 = sqrt((b->u_x * b->u_x) + (b->u_y * b->u_y));
  if (*direct != 0 && *direct != 180)
    d1 = my_find_dist_horizontal2(*a, pos, mapSize, map);
  if (*direct != 90 && *direct != 270)
    d2 = my_find_dist_vertical2(*b, pos, mapSize, map);
  free(a);
  free(b);
